...
HexGrid::HexGrid(int radius, std::vector<Hex> baseTiles) : radius_(radius) {
    if (radius_ < 0) {
...
std::vector<Hex> HexGrid::getOuterRing() const {
    ...
    // TODO: If we need speed, this could be optimized by doing this once
    // and saving the ring hexes as member variables.
