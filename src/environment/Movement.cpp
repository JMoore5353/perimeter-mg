...
geometry::Hex resolveSingleMove(const geometry::Hex& current, Action intended,
...
geometry::Hex resolveSingleMoveWithRoll(const geometry::Hex& current, Action intended,
                                        const geometry::Grid& grid, double roll)
{
  if (intended == Action::STAY) {
    return current;
  }

  // This doesn't seem super necessary since we clamp the pos after
                                                    ...
                                                    const geometry::Grid& grid, std::mt19937& rng)
{
  ...
  for (std::size_t index = 0; index < world.agents.size(); ++index) {
    nextPositions.push_back(
      resolveSingleMove(world.agents[index].position, jointActions[index], grid, rng));
  }

  return nextPositions;
}

void applySimultaneousMoves(core::WorldState& world, const std::vector<Action>& jointActions,
                            const geometry::Grid& grid, std::mt19937& rng)
{
  const std::vector<geometry::Hex> nextPositions =
    resolveIntendedPositions(world, jointActions, grid, rng);
